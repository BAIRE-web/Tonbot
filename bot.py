...
DATA_DIR = "data"
user_states = {}
user_progress = {}                                            user_states_avis = set()  # Pour suivre les utilisateurs en mode "avis"
...
def sauvegarder_utilisateur(user):
    ...
    if os.path.exists(chemin):
        with open(chemin, "r", encoding="utf-8") as f:
            users = json.load(f)
    uid = str(user.id)
    if uid not in users:                                              users[uid] = {"username": user.username or "", "nom": user.full_name or "", "bienvenue": False}
    else:                                                             users[uid]["username"] = user.username or users[uid].get("username", "")                                                    users[uid]["nom"] = user.full_name or users[uid].get("nom", "")
    with open(chemin, "w", encoding="utf-8") as f:
...
def generer_clavier(options):                                     return ReplyKeyboardMarkup([[opt] for opt in options], resize_keyboard=True)
    ...
    log_message(update.effective_user.id, f"Bot: {message}")      await update.message.reply_text(message, reply_markup=clavier)
...
claviers = charger_json("claviers.json")
    ...
    with open(chemin, "r", encoding="utf-8") as f:
        users = json.load(f)
    deja_accueilli = users[user_id].get("bienvenue", False)       nom = user.first_name or user.full_name or "cher utilisateur"
    ...
    if not deja_accueilli:
        ...
        with open(chemin, "w", encoding="utf-8") as f:
            json.dump(users, f, ensure_ascii=False, indent=2)
    else:                                                             msg = messages["retour"] if message_personnalise else messages["choix"]

    user_states[user.id] = "menu"
    user_progress.pop(user.id, None)
    log_message(user.id, "Commande /start")                       await repondre(update, msg, generer_clavier(claviers["menu_principal"]))

# --- Commande /avis pour que l'utilisateur envoie un avis ---async def avis_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    user_states_avis.add(user_id)                                 await update.message.reply_text("Quel est votre avis ou suggestion ?")
                                                              async def avis_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    if user_id not in user_states_avis:                               return False  # Pas en mode avis, on continue normalement
    ...
    user_states_avis.remove(user_id)                              await update.message.reply_text("Merci pour votre message !")
    ...
    if not msg:                                                       await repondre(update, "Utilise : /broadcast <message>")
        return
    chemin = os.path.join(DATA_DIR, "users.json")
    ...
    for uid in users:
        try:                                                              await context.bot.send_message(chat_id=int(uid), text=msg)
            ...
            pass                                                  await repondre(update, f"Message envoyé à {count} utilisateur(s).")
...
# --- Commande admin /listeavis pour voir tous les avis ---   async def listeavis(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != ADMIN_USER_ID:                     await repondre(update, "❌ Vous n'êtes pas autorisé à utiliser cette commande.")
        return
    chemin_avis = "avis.json"
    if not os.path.exists(chemin_avis):                               await repondre(update, "Aucun avis n'a encore été envoyé.")
        return
    with open(chemin_avis, "r", encoding="utf-8") as f:
        try:
            avis_list = json.load(f)
        except Exception:                                                 await repondre(update, "Erreur lors de la lecture des avis.")
            return

    if not avis_list:                                                 await repondre(update, "Aucun avis n'a encore été envoyé.")
        return
    messages_avis = []
    for avis in avis_list:                                            msg = f"👤 @{avis.get('username', 'inconnu')} (ID: {avis.get('user_id')}):\n" \
              f"📝 {avis.get('message')}\n" \
              f"📅 {avis.get('date')}\n\n"
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if await avis_message_handler(update, context):
        ...
        user_progress.pop(user_id, None)                              await start(update, context, message_personnalise=False)
        return                                                
    ...
    if user_id in user_states and user_states[user_id].startswith("qcm_"):
        ...
        if not qcm_data or "qcm" not in qcm_data:
            ...
            return
        index = user_progress.get(user_id, 0)                         question = qcm_data["qcm"][index]
        ...
        if texte_clean not in options_clean:
            await repondre(update, messages["choix_invalide"], generer_clavier(options + ["⬅️ Retour"]))                                 return
        ...
        user_scores[user_id]["nom"] = user.first_name                 user_scores[user_id]["actuel"]["total"] += 1
        ...
        save_user_scores()                                            return

    if texte in ["/start", "start", "demarrer", "démarrer"]:
        await start(update, context)                                  return

    if texte_original == "Quitter le bot":
        nom = user.first_name or user.full_name or "cher utilisateur"                                                               await repondre(update, messages["quitter"].replace("{nom}", nom), generer_clavier(["Démarrer"]))
        user_states.pop(user_id, None)
        user_progress.pop(user_id, None)                              return
    ...
    if texte in section_static:                                       cle = texte
        user_states[user_id] = cle                                    data = charger_json(section_static[cle])
    ...
    if texte in choix_sections:
        ...
        if texte == "technique":                                          await repondre(update, messages["technique_indisponible"], generer_clavier(["⬅️ Retour"]))
            return
    ...
    if user_id in user_states and user_states[user_id] == "concours":
        prefix = "concours"
        matiere = normaliser_nom(enlever_emojis(texte_original))                                                                    if matiere == "superieur_a_bac":
    ...
    if user_id in user_states and user_states[user_id] in choix_sections:
        prefix = normaliser_nom(user_states[user_id])                 matiere = normaliser_nom(enlever_emojis(texte_original))
        ...
        if "qcm" in qcm_data and qcm_data["qcm"]:
            ...
            user_progress[user_id] = 0                                    question = qcm_data["qcm"][0]
...
def save_user_scores():                                           chemin = "user_scores.json"
    with open(chemin, "w", encoding="utf-8") as f:
        json.dump(user_scores, f, ensure_ascii=False, indent=2)

user_scores = {}                                              if os.path.exists("user_scores.json"):
...
async def reset_score(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Sauvegarde dans l'historique avant reset                    historique.append({
        "date": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
    ...
    scores["historique"] = historique                             user_scores[user_id] = scores
    save_user_scores()
...
async def historique(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i, entry in enumerate(historique[-5:], 1):  # Affiche les 5 derniers
        total = entry["total"]
        correct = entry["correct"]                                    pourcentage = round((correct / total) * 100, 2) if total else 0
        msg += f"{i}. 📅 {entry['date']} - ✅ {correct}/{total} → {pourcentage}%\n"

    await update.message.reply_text(msg)

# --- Voir son propre score ---                               async def profil_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user
    uid = user.id
    ...
    msg += f"🎯 Précision : {pourcentage:.1f}%"                   await update.message.reply_text(msg, parse_mode="Markdown")
...
async def scores_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    count = 0
    for uid, data in user_scores.items():                             score = data.get("actuel", {})
        correct = score.get("correct", 0)                             total = score.get("total", 0)
        if total == 0:
            continue                                                  pourcentage = correct / total * 100
        nom = data.get("nom", f"ID:{uid}")
        msg += f"{nom} → {correct}/{total} ({pourcentage:.1f}%)\n"
        count += 1                                                    if count >= 30:  # limite d'affichage
            break
...
def lancer_bot():
    ...
    app.add_handler(CommandHandler("listeavis", listeavis))       app.add_handler(CommandHandler("reset_score", reset_score))
    app.add_handler(CommandHandler("historique", historique))

    # Ajout des commandes profil et scores
    app.add_handler(CommandHandler("profil", profil_command))     app.add_handler(CommandHandler("scores", scores_command))
...
if __name__ == "__main__":
    if not os.path.exists("logs"):
        os.makedirs("logs")
    threading.Thread(target=lancer_flask).start()                 lancer_bot()
